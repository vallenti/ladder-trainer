...
const ForRepsExerciseInput: React.FC<ForRepsExerciseInputProps> = ({
  exercise,
  onChange,
  onDelete,
  canDelete,
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.surface, shadowColor: theme.colors.shadow }]}>
      ...
      {/* Exercise Name + Reps Row */}
      <View style={styles.inputRow}>
        <TextInput
          mode="outlined"
          label="Reps per round"
          value={exercise.repsPerRound?.toString() || ''}
          onChangeText={(value) => {
            const numValue = parseInt(value, 10);
            onChange({ 
              ...exercise, 
              repsPerRound: isNaN(numValue) ? 0 : numValue 
            });
          }}
